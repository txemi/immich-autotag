...
def _is_proxy_module_import():
    ...
    for frame in stack:
        ...
        if filename.endswith("immich_autotag/api/immich_proxy/albums.py") or filename.endswith("immich_autotag/api/immich_proxy/__init__.py"):
            return True
    return False
...
class ArchitectureImportChecker(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path, target=None):
        ...
        if fullname.startswith(IMMICH_API_MODULE):
            if not _is_proxy_module_import():
                raise ImportError(f"Direct import of '{IMMICH_API_MODULE}' is forbidden. Only the proxy module may import it.")
