...
def compare_classification_tags(
    ...
    if tags1 == tags2:
        return _ClassificationTagComparisonResultObj(
            _ClassificationTagComparisonResult.EQUAL, None
        )
    if tags1 and not tags2 and len(tags1) == 1:
        return _ClassificationTagComparisonResultObj(
            _ClassificationTagComparisonResult.AUTOFIX_OTHER, next(iter(tags1))
        )
    if tags2 and not tags1 and len(tags2) == 1:
        return _ClassificationTagComparisonResultObj(
            _ClassificationTagComparisonResult.AUTOFIX_SELF, next(iter(tags2))
        )
    return _ClassificationTagComparisonResultObj(
        _ClassificationTagComparisonResult.CONFLICT, (tags1, tags2)
    )
