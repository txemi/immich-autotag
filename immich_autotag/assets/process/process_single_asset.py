...
@typechecked
def _analyze_and_assign_album(asset_wrapper: AssetResponseWrapper, tag_mod_report: ModificationReport, suppress_album_already_belongs_log: bool):
...
@typechecked
def _validate_and_update_classification(asset_wrapper: AssetResponseWrapper, tag_mod_report: ModificationReport):
    ...
    validate_and_update_asset_classification(asset_wrapper, tag_mod_report=tag_mod_report)
...
def process_single_asset(
    ...
    log(f"[DEBUG] [process_single_asset] START asset_id={getattr(asset_wrapper, 'id', None)}", level=LogLevel.FOCUS)
    ...
    _analyze_and_assign_album(asset_wrapper, tag_mod_report, suppress_album_already_belongs_log)
    ...
    log(f"[DEBUG] [process_single_asset] END asset_id={getattr(asset_wrapper, 'id', None)}", level=LogLevel.FOCUS)
