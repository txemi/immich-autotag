...
class DuplicateAlbumManager:
    """
    Encapsula toda la lógica de gestión de duplicados de álbumes:
    ...
    def handle_non_temporary_duplicate(self, *, existing: AlbumResponseWrapper, incoming_album: AlbumResponseWrapper, tag_mod_report: ModificationReport, name: str) -> None:
    ...
    def is_duplicated(self, wrapper: AlbumResponseWrapper) -> bool:
        ...
        names = self._collection.find_all_albums_with_name(name)
    ...
    @typechecked
    def _handle_duplicate_album_conflict(self, incoming_album: AlbumResponseWrapper, existing_album: AlbumResponseWrapper, context: str = "ensure_unique") -> AlbumResponseWrapper:
        ...
        if MERGE_DUPLICATE_ALBUMS_ENABLED:
            client = self._collection.get_client()
            tag_mod_report = self._collection.get_modification_report()
    ...
    @typechecked
    def _handle_non_temporary_duplicate(self, *, existing: AlbumResponseWrapper, incoming_album: AlbumResponseWrapper, tag_mod_report: ModificationReport, name: str) -> None:
        from immich_autotag.config.internal_config import MERGE_DUPLICATE_ALBUMS_ENABLED
        if MERGE_DUPLICATE_ALBUMS_ENABLED:
