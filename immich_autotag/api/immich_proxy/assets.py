...
def get_asset_info(asset_id: UUID, client: AuthenticatedClient) -> AssetResponseDto | None:
    """
    Wrapper centralizado para get_asset_info.sync.
    """
    return _get_asset_info.sync(id=asset_id, client=client)

def update_asset(asset_id: UUID, client: AuthenticatedClient, body: UpdateAssetDto) -> AssetResponseDto:
    """
